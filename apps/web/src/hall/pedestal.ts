...
/**
 * The pedestal standing between two displays.
 *
 * Scenery first and a loading state second — when the next payload is already
 * warm it should read as a column the visitor walked past, not as a spinner
 * that flashed. The pending cue is deliberately faint: a slow breath, nothing
 * more.
 *
 * Variants are picked by index rather than at random so a given stretch of
 * hall looks the same every time it is walked.
 */
