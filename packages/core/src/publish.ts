 ...
 * The publish bar.
 *
 * Signup is open and nobody is judged on their art. What a display must clear
 * before it can hang is objective: enough works, a description on each, and
 * images large enough to be worth looking at. That filters out empty and
 * careless displays — which is most of what makes an open platform feel cheap —
...
/**
 * Takes a display live.
 *
 * It becomes visible at the next epoch rather than instantly — that is the
 * cost of snapshot ordering. Sealing is enqueued immediately so the wait is
 * seconds rather than the full epoch interval.
 */
