 ...
 *
 * RDS presents a certificate signed by Amazon's own CA, which is not in Node's
 * trust store. Without this bundle, a verified connection fails with
 * "self-signed certificate in certificate chain" and the only way onward is to
 * turn verification off — a poor trade for a database that answers on the
 * public internet.
 *
 * Embedded as a string rather than shipped as a .pem on purpose. It has to
 * survive two bundlers (Next.js for the web tier, esbuild for the worker
 * Lambda), and neither copies data files without being told; the collage
 * assets already needed a special case for exactly that. This repository also
 * ignores *.pem, since that pattern is there to keep private keys out.
 *
 * 108 certificates, the global bundle covering every region. Refresh with:
 *   curl -o /tmp/rds-global.pem \
 *     https://truststore.pki.rds.amazonaws.com/global/global-bundle.pem
 * then re-run the generator noted in the commit that added this file.
 */
