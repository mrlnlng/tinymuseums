...
/* The collage compositor (worker-only): flattens an artist's hung works into ONE image per display with a region map, so hit testing runs against coordinates rather than per-piece geometry. */
